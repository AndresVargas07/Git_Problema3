...
public class CategoryRepository {
    ...
    public Category save(Category c) {
        if (c.getId() == null) { c.setId(nextId++); data.add(c); }
        else {
            for (int i = 0; i < data.size(); i++) {
                if (data.get(i).getId().equals(c.getId())) { data.set(i, c); }
            }
        }
        return c;
    }
    ...
    public Optional<Category> findById(Long id) { return data.stream().filter(x -> x.getId().equals(id)).findFirst(); }
}